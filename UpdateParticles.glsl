...
int pixelIndex() {
    float x = vUv.x * float(uParticleCount);
    return int(floor(x));
}

vec4 readParticle(int k) {
    float u = (float(k) + 0.5) / float(uParticleCount);
    return texture2D(uParticlesPrev, vec2(u, 0.5));
}
...
vec3 flow(vec3 p, float t) {
    vec3 q = p + t;
    return normalize(vec3(noise3(vec3(q.y, q.z, q.x)), noise3(vec3(q.z, q.x, q.y)), noise3(vec3(q.x, q.y, q.z))));
}
